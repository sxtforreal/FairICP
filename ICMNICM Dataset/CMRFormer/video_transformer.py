...
class VideoPatchEmbed(nn.Module):
    ...
    Input: (B, F, C, H, W)
    Output: (B*F, embed_dim, H, W)
    """

    def __init__(
        self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, num_frames=64
    ):
        super().__init__()
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(
        ...
        )  # remember to take pos_embed[1:] for tiling over time (1, patches_per_frame+1, embed_dim)
    ...
    def forward_features(self, x):
        ...
        x = self.patch_embed(x)  # (B*F, 768, 14, 14)
        x = x.flatten(2).transpose(2, 1)  # (B*F, 196, 768)
        x = x.reshape(b, -1, self.patch_embed.embed_dim)  # (B, F*196, 768)
