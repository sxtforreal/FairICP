...
def icmnicm_pred(data_dir, model_dir):
    ...
    with torch.no_grad():
        for index, batch in enumerate(dataloader):
            ...
            accessions, labels, videos = (
                batch["accessions"],
                batch["labels"].to(device),
                batch["videos"].to(device),
            )
            logits = model(videos)
            acc_list.extend(accessions)
            smx_list.append(torch.softmax(logits, dim=1).cpu())
            lbl_list.append(labels.cpu())

    smx_array = torch.cat(smx_list, dim=0).numpy()
    lbls_array = torch.cat(lbl_list, dim=0).numpy()
    phats = smx_array[:, 1]

    pred_df = pd.DataFrame(
        {
            "Accession_Number": acc_list,
            "Y": lbls_array,
            "Phat": phats,
        }
    )
